...
- [ ] **RTL layouts** — switch language to one of the 12 RTL codes (ar, ur, ps, he, fa, prs, arz, mey, sd, ckb, aii, rhg). The wordmark, mobile tab bar, side rail brand mark, special-dispatch frame, and §N markers should all render correctly with `dir="rtl"`. Italic-serif body copy should flow right-to-left.
- [ ] **i18n recommended-path string** — switch through all 76 locales and confirm the recommended route card shows the localized "Recommended Path" string. Spot-check Cyrillic (ru, uk, sr, mn, kk), CJK (zh, yue, ja, ko, nan), Indic (hi, gu, pa, ne, ur, bn, mr, ta, te, ml, kn, or, mai, bho, si, sd), Ethiopic (am, ti), Myanmar/Karen (my, ksw), and RTL (ar, ps, ur, he, fa, prs, arz, mey, sd, ckb, aii, rhg) for proper rendering.
- [ ] **LocaleExpansion in-browser verification** — flip `VITE_CONTINENT_PICKER_ENABLED=true` in a Vercel preview, walk through `my, am, bn, ta, th, km, he, ckb, si, mr` to confirm no tofu boxes (Chunk 13 font integration), and confirm the continent picker keyboard + screen-reader flow on at least one RTL locale (Chunk 15 acceptance #4–5).
- [ ] **Native-speaker translation review for `mey` and `ceb`** — the machine-translated `mey` (Hassaniya Arabic) reads as MSA rather than the Mauritanian dialect; `ceb` (Cebuano) has Tagalog tinges. Both surface the machine-translated review badge with a feedback link, but tracking here so the issue isn't forgotten if no riders surface it.
