...
- [ ] **RTL layouts** — switch language to one of the 6 active RTL codes (ar, ur, ps, prs, aii, rhg). The wordmark, mobile tab bar, side rail brand mark, special-dispatch frame, and §N markers should all render correctly with `dir="rtl"`. Italic-serif body copy should flow right-to-left.
- [ ] **i18n recommended-path string** — switch through all 27 active locales and confirm the recommended route card shows the localized "Recommended Path" string. Spot-check Cyrillic (ru, uk), CJK (zh, yue, ko), Indic (hi, gu, ne, ur), Ethiopic (am), Karen (ksw), and RTL (ar, ps, ur, prs, aii, rhg) for proper rendering.
- [ ] **LocaleExpansion in-browser verification** — flip `VITE_CONTINENT_PICKER_ENABLED=true` in a Vercel preview, walk through the non-Latin active locales (`am`, `ksw`, `rhg`, `aii`, `hi`, `gu`, `ne`, `ur`, `ar`, `ps`, `prs`, `zh`, `yue`, `ko`) to confirm no tofu boxes (Chunk 13 font integration), and confirm the continent picker keyboard + screen-reader flow on at least one RTL locale (Chunk 15 acceptance #4–5).
